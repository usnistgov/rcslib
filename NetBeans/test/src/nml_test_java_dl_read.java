...
public class nml_test_java_dl_read
{
	...
	try {
	    ...
	    if(args.length < 3)
		{
		    System.err.println("nml_test_java_dl_read usage: <buffername> <processname> <configfile> <debug> <repeat_count> <delay>");
		    System.exit(1);
		}
	    try
		{
	    ...
	    if(new_count > 0) {
		System.out.printf("Timing info new : count=%d, min=%.3f,max=%.3f,avg=%.3f\n",
				  old_count,new_min,new_max,new_total/new_count);

	    }
	    if(old_count > 0) {
		System.out.printf("Timing info old : count=%d,min=%.3f,max=%.3f,avg=%.3f\n",

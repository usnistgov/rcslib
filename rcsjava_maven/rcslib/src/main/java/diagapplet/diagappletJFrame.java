...
//==============================================================================
class diagappletJFrame extends javax.swing.JFrame implements WindowListener, ComponentListener
{
     ...
     */
    private static final long serialVersionUID = 2613927L;
	...
	try
		...
		if(screen_d.width > 600 &&
		   new_width > screen_d.width+10)
		    {
			return;
		    }
		if(screen_d.height > 400 &&
		   new_height > screen_d.height+10)
		    {
			return;
		    }
		if((screen_d.width > 700 || screen_d.width < 600) &&
		   (screen_d.height > 640 || screen_d.height < 400))
		    {
			if(new_width < 700 || new_height < 600)
			    {
				if(new_width < 700)
				    {
					new_width = 700;
		...
		if(debug_on)
		    {
			System.out.println("prevSize="+prevSize+",new_width="+new_width+", new_height="+new_height);
		    }
		if((Math.abs(prevSize.width - new_width) > 5 ||
		   ...
		   force_it)
			...
			if(true)
			    {
				System.out.println("DiagappletFrame resizing to "+new_width+"X"+new_height+" : Screen="+d.width+"x"+d.height);
			    }
			...
			if(null != innerApplet)
				...
				getContentPane().add("Center",innerApplet);
			    }
			validate();
			if(null != innerApplet)
			    {

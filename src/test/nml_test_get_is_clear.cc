...
main(int argc, char **argv)
{
  ...
  if(argc < 5)
    {
      fprintf(stderr,"usage : bufname procname configfile is_clear\n");
      exit(255);
    }
  ...
  int t;
  int is_clear = strtol(argv[4],0,0);
  if((t = nml->get_is_clear()) != is_clear)
    {
      fprintf(stderr,"nml->get_is_clear() returned %d\n", t);
      delete nml;
      exit(255);
    }
  delete nml;

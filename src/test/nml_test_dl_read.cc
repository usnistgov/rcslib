...
static int
check_test_message(TEST_MESSAGE *tst_msg, int expected_last_var,NML *nml)
{
  int bads=0;
 ...
 //this should have been sent and should be 3.33
   ...
   * FIXME: long long are more trouble to support than they are worth.
   ...
   printf("GOOD tst_msg->ll = %Ld\n",(-1LL*0x123456789LL));
   } else {
   printf("BAD tst_msg->ll = %Ld instead of %Ld \n",
   tst_msg->ll,
   (-1LL*0x123456789LL));
   bads++;
   }
   ...
   printf("GOOD tst_msg->ull = 0x%LX\n",(0x123456789ULL));
   } else {
   printf("BAD tst_msg->ull = 0x%LX instead of 0x%LX \n",
	   tst_msg->ull,
	   (0x123456789ULL));
    bads++;
    }
    ...
    printf("GOOD tst_msg->lla[0] = %Ld\n",(-1LL*0x123456789LL));
    } else {
    printf("BAD tst_msg->lla[0] = %Ld instead of %Ld \n",
    tst_msg->lla[0],
    (-1LL*0x123456789LL));
    bads++;
  }
  ...
  if(tst_msg->ulla[0] = 0x123456789ULL) {
    printf("GOOD tst_msg->ulla[0] = 0x%LX\n",(0x123456789ULL));
  } else {
    printf("BAD tst_msg->ulla[0] = 0x%LX instead of 0x%LX \n",
	   tst_msg->ulla[0],
	   (0x123456789ULL));
    bads++;
  }
...
main(int argc, char **argv)
{
  ...
  if(!nml->valid())
    {
      fprintf(stderr,"nml->valid() check failed. error_type=%d, cfgsvr_status=%d\n",nml->error_type, nml->getCfgSvrStatus()); 
      delete nml;
      exit(255);
    }
  int t;
  if(TEST_MESSAGE_TYPE != (t = nml->read()))
    {
      fprintf(stderr,"nml->read() returned %d\n", t);
      delete nml;
      exit(255);
    }

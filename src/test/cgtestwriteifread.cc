...
#ifdef VXWORKS
extern "C" void example_nml_write_if_read();

void example_nml_write_if_read()
#else
...
char *MY_LOG_FILE=0;
  ...
  MY_LOG_FILE = getenv("CGTESTWRITEIFREAD_LOG_FILE");
  if(MY_LOG_FILE)
    {
  ...
  if(selflags[0] == 'X')
    {
      ...
      t1=etime();
      if( example_nml1l->write_if_read(example_msg) < 0) 
	{
	  fprintf(stderr, "example_nml1l->write_if_read() failed\n");
      ...
      t2 = etime();
      printf("example_nml1l->write_if_read(example_msg) took %f seconds.\n",(t2-t1));
    }
  //  example_nml2l->write_if_read(example_msg);
  ...
  if(selflags[1] == 'X')
    {
      ...
      printf("cgtestwrite : example_msg->lastvar = %d\n",example_msg->lastvar);
      t1=etime();
      if( example_nml2r->write_if_read(example_msg) < 0)
	{
	  fprintf(stderr, "example_nml2r->write_if_read() failed\n");
	  delete example_nml1l;
	  delete example_nml2r;
	  delete example_msg;
#ifdef DEBUG_MEMORY
      ...
      t2 = etime();
      printf("example_nml2r->write_if_read(example_msg) took %f seconds.\n",(t2-t1));
    }
